...
it('supports async zod v4 refinements', async () => {
  const hook = renderHook(() =>
    useForm({
      initialValues: {
        username: 'taken',
...
it('supports explicit async mode with a sync zod v4 schema', async () => {
  const hook = renderHook(() =>
    useForm({
      initialValues: {
        name: '',
...
it('throws in sync mode when zod v4 schema has async refinements', () => {
  const hook = renderHook(() =>
    useForm({
      initialValues: {
        username: 'taken',
